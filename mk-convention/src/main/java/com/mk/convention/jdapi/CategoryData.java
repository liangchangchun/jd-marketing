...
public class CategoryData  extends BaseDataEvent implements DataEvent,java.io.Serializable{
	/**
	 * 
	 */
	private static final long serialVersionUID = 7556581133133744869L;
 ...
 */
	...
	public void ApiRequest(JdDataEvent jdDataEvent,Disruptor<JdDataEvent> disruptor,JdDataEventTranslator eventTranslator) {
		...
		public void adapterEs(JdDataEvent jdDataEvent) {
			CategoryData dataEvent = (CategoryData)jdDataEvent.getEvent();
			JSONArray skuids = dataEvent.getSkuIds();
	        if(skuids==null){
	            return;
	        }
	    	System.out.println("adapterEs########skuIds:"+skuIds);
	        String category_id = dataEvent.getCategoryId();
	        List<CategoryDocument> categorys = new ArrayList<CategoryDocument>();//最终获取数据
	        String skuId = "";
	        for(int jj = 0 ;jj < skuids.size();jj++){
	        	skuId = skuids.get(jj).toString();
	        	CategoryDocument category = new CategoryDocument();
	            category = new CategoryDocument();
	            category.setSkuId(skuId);
	            category.setCategoryId(category_id);
	            categorys.add(category);
	            //插入索引
	            //jdDataEvent.getEsRes().save(category);
	        }
	        jdDataEvent.getEsRes().saveAll(categorys);
		}
	/**
	 * 数据插入逻辑
	 */
